...
async function canBattle(userId) {
    // Ensure the battles file exists before trying to read it
    ...
    const battlesToday = battles.filter(battle => {
        // Check if the user was either the winner or the loser
        const isParticipant = battle.winnerId === userId || battle.loserId === userId;
...
async function runBattle(player1Id, player2Id) {
    try {
        ...
        const favoritesP2 = await fs.readJson(`./src/data/favorites/${player2Id}.json`);

        if (favoritesP1.length === 0 || favoritesP2.length === 0) {
            return { error: "One or both players have no favorite Pokémon." };
        }

        const randomPokemonP1 = favoritesP1[Math.floor(Math.random() * favoritesP1.length)];
        const randomPokemonP2 = favoritesP2[Math.floor(Math.random() * favoritesP2.length)];

        const p1DataPromise = axios.get(`https://pokeapi.co/api/v2/pokemon/${randomPokemonP1.id}`);
        const p2DataPromise = axios.get(`https://pokeapi.co/api/v2/pokemon/${randomPokemonP2.id}`);
        ...
        const loserId = score1 < score2 ? player1Id : player2Id;

        // This part now also serves as our battle recorder.
        const battles = await fs.readJson(BATTLES_FILE, { throws: false }) || [];
        battles.push({
            winnerId,
            loserId,
        ...
        switch (data.type) {
            case 'challenge': {
                ...
                if (!await canBattle(challengerId)) {
                    ws.send(JSON.stringify({ type: 'battle_error', message: 'You have reached your daily battle limit of 5 and cannot challenge others.' }));
                    return;
                }
            ...
            case 'challenge_accepted': {
                const player1Id = data.fromId; // Challenger
                const player2Id = data.toId;   // Opponent who accepted

                const player1 = onlineUsers.get(player1Id);
                const player2 = onlineUsers.get(player2Id);

                // --- BATTLE LIMIT CHECK FOR BOTH PLAYERS ---
                if (!await canBattle(player1Id)) {
                    const msg = JSON.stringify({ type: 'battle_error', message: `Battle canceled. ${player1.username} has reached their daily battle limit.` });
                    if (player1) player1.ws.send(msg);
                    if (player2) player2.ws.send(msg);
                    return;
                }
                if (!await canBattle(player2Id)) {
                    const msg = JSON.stringify({ type: 'battle_error', message: `Battle canceled. You have reached your daily battle limit.` });
                    if (player2) player2.ws.send(msg); // Send specific message to player 2
                    if (player1) player1.ws.send(JSON.stringify({ type: 'battle_error', message: `Battle canceled. ${player2.username} has reached their daily limit.` }));
                    return;
                }

                if (player1 && player2) {
                    console.log(`Challenge accepted. Running battle for ${player1.username} and ${player2.username}.`);
            ...
            // --- EXAMPLE: BOT BATTLE IMPLEMENTATION ---
            case 'start_bot_battle': {
                ...
                // 1. Pick a random bot Pokémon
                // We'll use the same getRandomPokemon function as in runBattle
                const botPokemon = await getRandomPokemon();

                // 2. Run the battle calculation
                // We'll simulate the bot as a player with id 'BOT' and username 'Bot'
                // The player is always player1, bot is player2
                const player = onlineUsers.get(playerId);
                if (!player) {
                    ws.send(JSON.stringify({ type: 'battle_error', message: 'Player not found.' }));
                    return;
                }

                // Get a random Pokémon for the player
                const playerPokemon = await getRandomPokemon();

                // Calculate stats for both Pokémon
                function getStat(stats, name) {
                    const statObj = stats.find(s => s.stat.name === name);
                    return statObj ? statObj.base_stat : 0;
                }

                // Simple battle logic: sum of (attack + defense + speed + hp)
                const playerScore =
                    getStat(playerPokemon.stats, 'attack') +
                    getStat(playerPokemon.stats, 'defense') +
                    getStat(playerPokemon.stats, 'speed') +
                    getStat(playerPokemon.stats, 'hp');

                const botScore =
                    getStat(botPokemon.stats, 'attack') +
                    getStat(botPokemon.stats, 'defense') +
                    getStat(botPokemon.stats, 'speed') +
                    getStat(botPokemon.stats, 'hp');
                ...
                if (playerScore > botScore) {
                    ...
                    // In case of a tie, randomly pick winner
                    if (Math.random() < 0.5) {
                        winnerId = playerId;
                        loserId = 'BOT';
                    } else {
                ...
                // 3. Record the battle in battles.json for the player
                // Only record if player is not 'BOT'
                const battleRecord = {
                ...
                // Save to battles.json
                try {
                    const battles = await fs.readJson('./src/data/battles.json');
                    battles.push(battleRecord);
                    await fs.writeJson('./src/data/battles.json', battles, { spaces: 2 });
                } catch (err) {
                    ws.send(JSON.stringify({ type: 'battle_error', message: 'Failed to record battle.' }));
                    return;
                }
                ...
                ws.send(JSON.stringify({
                    type: 'battle_ready',
                    player1: { id: playerId, username: player.username },
                ...
                ws.send(JSON.stringify({ type: 'battle_error', message: 'Bot battles are not fully implemented yet.' }));
